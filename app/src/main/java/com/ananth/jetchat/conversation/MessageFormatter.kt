...
fun MessageFormatter(
    ...
    return buildAnnotatedString {
        ...
        val codeSnippetBackground =
            if (primary) MaterialTheme.colorScheme.secondary else MaterialTheme.colorScheme.surface

        for (token in tokens) {
            append(text.slice(cursorPosition until token.range.first))

            val (annotatedString, stringAnnotation) = getSymbolAnnotation(
                matchResult = token, colorScheme = MaterialTheme.colorScheme, primary = primary,
                colorSnippetBackground = codeSnippetBackground
            )
            append(annotatedString)
            if (stringAnnotation != null) {
                val (item, start, end, tag) = stringAnnotation
                addStringAnnotation(tag, item, start, end)
            }
            cursorPosition = token.range.last + 1
        }
        if (!tokens.none()) {
            append(text.slice(cursorPosition..text.lastIndex))//remaining text from cursor position to last index of user entered text
        } else {
            append(text)
        }
    }
        ...
        '`' -> SymbolAnnotation(
            AnnotatedString(
                ...
                spanStyle = SpanStyle(
                    ...
                    fontSize = 12.sp,
                    background = colorSnippetBackground,
